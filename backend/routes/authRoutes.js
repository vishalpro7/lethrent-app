...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    console.log(`✅ Step 1: User created successfully. New user_id: ${userId}`);

    if (role === "owner") {
      await db.query("INSERT INTO owners (user_id, license_number) VALUES ($1, $2)", [userId, "TEMP_LICENSE"]);
      console.log(`✅ Step 2: Owner profile created for user_id: ${userId}`);
    } else {
      await db.query("INSERT INTO renters (user_id, id_proof) VALUES ($1, $2)", [userId, "TEMP_ID"]);
      console.log(`✅ Step 2: Renter profile created for user_id: ${userId}`);
    }

    res.json({ success: true, message: "Signup successful", user_id: userId, role });
  } catch (err) {
    // THE FIX: Log the ENTIRE error object for full details
    console.error("❌ DETAILED ERROR DURING SIGNUP:", err); 
    res.status(500).json({ success: false, error: "An internal server error occurred." });
  }
});

// LOGIN (no changes needed here, but keeping it for completeness)
